export const collegeNames  = ["A. D. Patel Institute of Technology",
...
export const searchColleges = (query: string): string[] => {
  ...
  // Filter colleges that contain the search query
  const filteredColleges = collegeNames.filter(college =>
    college.toLowerCase().includes(lowercaseQuery)
  );

  // Sort results: exact matches first, then partial matches
  const sortedResults = filteredColleges.sort((a, b) => {
    const aLower = a.toLowerCase();
    const bLower = b.toLowerCase();
    
    // Exact match at start
    if (aLower.startsWith(lowercaseQuery) && !bLower.startsWith(lowercaseQuery)) {
      return -1;
    }
    if (!aLower.startsWith(lowercaseQuery) && bLower.startsWith(lowercaseQuery)) {
      return 1;
